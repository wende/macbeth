...
- Window outline and synthetic pointer appear only when the targeted window is system frontmost at the start of the operation. Background inspect/manipulate skips those. If the app loses frontmost *outside* an activity scope, outlines drop immediately. Paths that must activate (`press_key` / `press_keys`, keyboard `fill`, Electron auto-fill) re-present the outline after activation. The mouse-click raise-and-restore path does not re-present: it sends `targetActivated` so any buffered outline from a previous app is retired before the brief raise, then restores the previous frontmost app. Screenshot capture scan/snap still runs while backgrounded so demos and external recordings can show which window is being captured (e.g. when a screen recorder holds frontmost).
- Sequential tools open an activity scope *before* target resolution so a preceding outline can be re-armed instead of blanking mid hand-off. While a scope is active, frontmost reconciliation is suppressed: a user Cmd+Tab mid-tool may leave a brief stale outline until the tool ends and the normal debounce fade runs. That is intentional continuity, not a leak. Daemon-driven activates that will not send `focusWindow` use `targetActivated` instead so they do not inherit that buffer.
- Outlines are per stable process/window identity; several windows of the frontmost app can stay outlined. Only the frontmost app has chrome. A `focusWindow` whose owner is not the current frontmost app is ignored (out-of-order concurrent delivery must not wipe legitimate chrome). After the last overlapping operation, each outline holds 400ms then fades 100ms. Re-addressing refreshes the deadline without restarting fade-in.
